...
浏览器打开 `http://<NAS-IP>:8090/` 进入管理面板，**输入管理密码后登录**（密码为 `MIGU_ADMIN_PASSWORD`，默认 `admin`，请修改）。面板包含：

- **订阅地址**：跳转版 / 直链版两个 M3U 地址，一键复制；
- **系统设置**（右上角 ⚙）：自定义面板标题、上传登录页壁纸（自动高斯模糊 + 暗色蒙版，可随时移除）；
...
> ⚠️ 面板的扫码登录入口会先弹出风险提示，需点“我已知晓”才会出现二维码；**作者尚未完整跑通该流程**，且登录存在封号风险，请谨慎使用。
...
| `MIGU_TITLE` | `migu-m3u 管理面板` | 面板默认标题（登录后可在设置中修改） |
