...
基于咪咕视频公开接口开发的免登录动态 M3U 服务。你只需要在 NAS 上跑一个容器，把生成的 M3U 地址填进 APTV 等播放器，就能看 CCTV、各大卫视和地方台的直播。

**为什么不用央视项目了？** 央视直播流是私有加密的（上一版已经确认无法直接播放）。咪咕视频的直播流是标准 HLS（m3u8 + TS），本服务已实测可以正常拉到 TS 分片，播放器直接可播。
